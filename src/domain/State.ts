...
export class State {
  ...
  piecesAt(location: Coordinates): Piece[] {
  ...
  moveActiveTo(location: Coordinates): void {
    this.piecesAt(location).map((p) => p.kill());
  }
}
...
const getInitialState = (): Piece[] => [
  ...
  new Piece(PieceType.King, Color.White, { x: 3, y: 0 }),
  new Piece(PieceType.Queen, Color.White, { x: 4, y: 0 }),
  ...
  new Piece(PieceType.King, Color.Black, { x: 3, y: 7 }),
  new Piece(PieceType.Queen, Color.Black, { x: 4, y: 7 }),
];
